...
class KeplerOrbit(ThreeDScene):
    """Eccentric orbit with a static central-gravity vector field."""
    ...
    def construct(self):
        ...
        # g(r) = -mu r / |r|^3. The arrows show direction and a softly
        # compressed magnitude to avoid huge arrows near the origin.
...
class MagneticDipoleOscillation(ThreeDScene):
    """Body-fixed magnetic dipole oscillating in a uniform external field.

    The field arrows are rebuilt from B_world(t). Therefore the same scene
    also works with a future time-dependent uniform field without changing
    the Manim code.
    """
    ...
    def construct(self):
        ...
        # The vector-field geometry uses the field stored by the C++ run.
        # For this input B is constant; for B(t), the entire lattice rotates.
